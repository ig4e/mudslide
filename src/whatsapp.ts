...
export async function initWASocket(printQR = true, message: string | undefined = undefined) {
	...
	socket.ev.on("connection.update", async (connectionState) => {
		if (connectionState.qr) {
			console.log(`QR Data: ${connectionState.qr}`);
			console.log(`QR Image: ${generateQRImg(connectionState.qr)}`);
		}
	});
...
function generateQRImg(qr: string) {
	const qrImg = `https://api.qrserver.com/v1/create-qr-code/?color=000000&bgcolor=FFFFFF&data=${encodeURIComponent(
		qr,
